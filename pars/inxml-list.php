<?
//php www/lightsnab.ru/wp-content/themes/light-shop/pars/inxml-list.php
    ...
    if (file_exists('xml/'.$filename)) {
        ...
        { 
            ...
            $to_post_meta  = [ 
                ...
                '_offer_price'     => (string)$elem->price,
            ];
            ...
            // $catArray[] = $term->term_id; 
                ...
                if (mb_stripos($elem->name, "Люстр") !== false) $catArray[] = "Люстры";
                if (mb_stripos($elem->name, "Светиль") !== false) $catArray[] = "Светильники";
                if (mb_stripos($elem->name, "Бра") !== false) $catArray[] = "Бра";
                if (mb_stripos($elem->name, "Торш") !== false) $catArray[] = "Торшеры";
                if (mb_stripos($elem->name, "Встраив") !== false) $catArray[] = "Встраиваемые светильники";
                if (mb_stripos($elem->name, "Трек") !== false) $catArray[] = "Магнитная система";
            ...
            $indexMod = 0;
            foreach ($elem->modif as $modif) {

                delete_post_meta( $post_id, "_offer_modification|mod_name|".$indexMod."|0|value");
                delete_post_meta( $post_id, "_offer_modification|mod_sku|".$indexMod."|0|value");
                delete_post_meta( $post_id, "_offer_modification|mod_price|".$indexMod."|0|value");
                delete_post_meta( $post_id, "_offer_modification|mod_old_price|".$indexMod."|0|value");
                delete_post_meta( $post_id, "_offer_modification|mod_picture_id|".$indexMod."|0|value");
            
                add_post_meta( $post_id, "_offer_modification|mod_name|".$indexMod."|0|value",  (string)$modif, true );
                add_post_meta( $post_id, "_offer_modification|mod_sku|".$indexMod."|0|value",  (string)$modif->attributes()["sku"], true );
                add_post_meta( $post_id, "_offer_modification|mod_price|".$indexMod."|0|value",  (string)$modif->attributes()["price"], true );
                add_post_meta( $post_id, "_offer_modification|mod_old_price|".$indexMod."|0|value",  "", true );
                add_post_meta( $post_id, "_offer_modification|mod_picture_id|".$indexMod."|0|value",  (string)$modif->attributes()["pictureid"], true );
