...
function add_zak( WP_REST_Request $request ){
	...
	for ($i = 0; $i<count($zakinfo["zaktovars"]); $i++)
	{
		$serviceBase->insert('zakaz_tovar', array(
			...
			"cerecter" => $zakinfo["zaktovars"][$i]["cerecter"],
...
function update_zak( WP_REST_Request $request ){
	...
	for ($i = 0; $i<count($zakinfo["zaktovars"]); $i++)
	{
		$result = $serviceBase->insert('zakaz_tovar', array(
		...
		if (!empty($result)) $chengeTovar = true;
	}

	if (empty($updateRez) && empty($result)) {
...
function sale_report( WP_REST_Request $request ){
	...
	$q = "SELECT `zakaz`.* FROM `zakaz` WHERE (`status` = 'Новый' OR `status` = 'Архив') AND (`zak_final_data` >= '".$start."' AND `zak_final_data` <= '".$end."') AND `mng_mail` LIKE '".$manager."'";
